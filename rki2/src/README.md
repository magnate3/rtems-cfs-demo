...
```
[root@centos7 src]# export PATH=$HOME/development/rtems/compiler_arm/6/bin:$PATH
[root@centos7 src]# export RTEMS_MAKEFILE_PATH=$HOME/development/rtems/compiler_arm/6/arm-rtems6/xilinx_zynq_a9_qemu
...
```
[root@centos7 src]# qemu-system-arm -serial null -serial mon:stdio -nographic   -M xilinx-zynq-a9 -m 256M   -net tap,ifname=qtap,script=no,downscript=no   -net nic,model=cadence_gem,macaddr=0e:b0:ba:5e:ba:12 -kernel rki.elf.pre
qemu-system-arm: could not load kernel 'rki.elf.pre'
[root@centos7 src]# qemu-system-arm -serial null -serial mon:stdio -nographic   -M xilinx-zynq-a9 -m 256M   -net tap,ifname=qtap,script=no,downscript=no   -net nic,model=cadence_gem,macaddr=0e:b0:ba:5e:ba:12 -kernel o-optimize/hello.ralf 
